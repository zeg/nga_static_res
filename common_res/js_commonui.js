		...
		function(){
			...
			for(var i=0;i<b.length;i++){
				...
				if(!hp || !hp[1].match(/\.(ngacn\.cc|nga\.cn|178\.com)$/))
					return alert('只能使用站内图片 (ngacn.cc/nga.cn/178.com)')
...
if(a==1)
	...
	y = ['deeppink','LOCKED','锁定','因账号安全问题被禁用','使用[邮箱重置密码]解锁, 或使用论坛app中的[忘记密码]重置密码解锁(绑定手机时间需在锁定时间之前)','https://account.178.com/?p=reset_pass']
else if(a==-2)
